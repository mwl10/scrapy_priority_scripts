...
class CrystalbridgesSpider(Spider):
    name = 'crystalbridges'
    start_urls = ['https://crystalbridges.org/exhibitions/']

    def parse(self, response):
        exhibits = response.css('.photo_box.pb_left')
        for exhibit in exhibits:
            url = exhibits[0].css('a::attr(href)').get()
